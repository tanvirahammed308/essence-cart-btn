...
const PopularProduct = ({products,handleAddToCart}) => {
    const filterNewArrival=products.filter((item)=>item.productDisplayCategory ==="Popular Product")
    

  return (
    <div className='my-5  '>
    <h1 className='text-2xl text-center md:text-left font-semibold my-8 text-[#4F5C69] capitalize 
 '><span className='text-[#5CAF90]'>popular</span>  products </h1>
    <div className='grid md:grid-cols-4 gap-7 justify-center   '>
      {
          filterNewArrival.map((item)=><div className="card bg-base-100 w-72 shadow-sm ">
          <figure>
            <img
